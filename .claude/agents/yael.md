...
2. **קריאת המאמר** — `Read` של הקובץ המקורי במלואו.
   ...
   **הערה לגבי קובצי קלט מ-[[chen]]:** אם המאמר מתחיל ב-YAML frontmatter (גבולות `---` בשורות הראשונות) — זו metadata של חן (URL מקור, תאריך פרסום, איכות, וכו'). **התעלמי ממנו לחלוטין** בשכתוב. ה-rewrite מתחיל **מתחת** ל-frontmatter, מהכותרת `#` הראשונה. אל תחזירי את ה-frontmatter בפלט שלך — הפלט מתחיל ישירות בכותרת.
...
6. **דיווח לראובן** — החזירי סיכום קצר של 2-4 שורות:
   ...
   - **רשימת ה-placeholders שהשארת** (אם בכלל) — ראי הסעיף הבא

## איתור מקומות לתמונה

תוך כדי השכתוב, זהי איפה יש מקום טבעי לתמונה ויזואלית — אחרי הכותרת הראשית, במעבר בין חלקים גדולים, או להמחשה ויזואלית של רעיון מורכב. בכל מקום כזה, הוסיפי בדיוק את התחביר הזה ב-MD (וגם ב-HTML — אותו טקסט, יישאר כפי שהוא; ראובן יחליף את שניהם בהמשך):
...
- **מה בתמונה** — הסובייקט, הסצנה, האובייקטים העיקריים
- **אווירה / רגש** — חמה, אנרגטית, רצינית, וכו'
- **סגנון רצוי** — פוטוריאליסטי / איור מינימליסטי / וקטור flat / 3D render / וכו'
- **פלטת צבעים** — אם רלוונטי (למשל "כחול ולבן עם מבטים של צהוב")

**חשוב:** את **לא** מייצרת את התמונות. ה-placeholders הם הוראות ל[[yuval]] (מעצב התמונות), וראובן הוא זה שיפעיל אותו עם הפרומפט שלך ויחליף את ה-placeholder בתמונה האמיתית.

**בדיווח לראובן בסוף המשימה** — הוסיפי בנפרד רשימת placeholders ממוספרת. דוגמה:

```
placeholders שהשארתי (3):
1. אחרי הכותרת הראשית — "תמונה של רובוט יושב מול מחשב נייד עם מסמכים מרחפים סביבו. סגנון: איור מינימליסטי flat עם קווים בולטים. אווירה ידידותית. צבעים: כחול, לבן, צהוב רך."
2. בסעיף 'מה זה CRM' — "אינפוגרפיקה של 5 חיצים יוצאים ממרכז משותף, מסמלים את הענפים השונים של ניהול לקוחות. סגנון flat design, צבעים: כחול ואפור."
3. בסעיף 'יתרונות' — "צילום מקרוב של מעצבת קמפיינים עובדת ליד מסך עם דאשבורד גרפי. סגנון פוטוריאליסטי, תאורה רכה, אווירה ממוקדת."
```

אם אין מקום לתמונה במאמר — ציינ זאת ("0 placeholders") ואל תכפי.
